...
inventorNames=inventors.map((inventor)=>{
    let object={first: inventor.first, last: inventor.last };
    return object
});
...
sortedByBirthYear=inventors.sort((a,b)=>{
    return a.year - b.year // found this method online on MDN docs and w3 schools but not very sure exxactly how it works.
});
...
inventorNamedAda=inventors.find((inventor)=>{
    return inventor.first === "Ada" || inventor.last==='Ada'
})
...
isAdultPresent=devs.some((dev)=>{
    return thisYear.getFullYear()-dev.year>18
})
...
isEveryone19OrOlder=devs.every((dev)=>{
    return thisYear.getFullYear()-dev.year >=19
})
